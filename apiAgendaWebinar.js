...
async function createRoom(event, userdata, meetingdetails) {
  ...
  console.log(" webinar meeting details", meetingdetails)
  console.log("event.common.event.event_info", event.common.event.event_info);
  ...
  if (
    ...
    roomConfig = await JSON.parse(
      meetingdetails.meeting_details.apidetails.apidetails
        .roomConfiguration.room_config
    );
  } else {
    
    roomname = await meetingdetails.meeting_details.apidetails.apidetails.roomConfiguration
        .room_name;
    roomConfig = await JSON.parse(
      meetingdetails.meeting_details.apidetails.apidetails
        .roomConfiguration.room_config
  ...
  return await Axios.post(
    "https://api.daily.co/v1/meeting-tokens",
    {
      properties: {
        ...
        is_owner: true,
        start_video_off: (meetingdetails.meeting_details.apidetails.select_speakers.indexOf(
          userdata.common.user.id.toString() 
        ) > -1 || event.common.event.event_info.user_role == 'presenter')?false:true,
        start_audio_off: (meetingdetails.meeting_details.apidetails.select_speakers.indexOf(
          userdata.common.user.id.toString()
        ) > -1 || event.common.event.event_info.user_role == 'presenter')?false:true,
