...
const ExhibitorsFav = (props) => {
  ...
  const [favouritesData, setFavourites] = useState([]);
  ...
  useEffect(() => {
    ...
    Axios.post(`${api_url.myFavourites}`, formData).then((res) => {
      console.log(store.getState().login.cookie);
      console.log(res.data);
      setFavourites(res.data.attendees ? res.data.attendees : []);
    ...
    <ScrollView style={styles.container}>
      <View style={styles.cardsWrapper}>
        {isLoading ? (
          ...
          favouritesData.map((fav, i) => (
            <TouchableOpacity
              key={i}
              onPress={() =>
                navigation.navigate("userDetails", { user_id: fav.attendee_id })
              }
            >
...
export default connect(mapStateToProps)(ExhibitorsFav);
