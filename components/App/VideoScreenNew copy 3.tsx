...
const VideoScreenNew = (props) => {
  ...
  useEffect(() => {
    ...
    createRoom(props.login, props.route.params.meeting_id.meeting_id, props.route.params.token);
  ...
  useEffect(() => {
    ...
    .then(()=>{
      if(waitingListClearInterval) clearInterval(waitingListClearInterval);
      waitingParticipants = [];
      setInterval(()=>{ 
        const waitingListFromCallObject = Object.keys(callObject._waitingParticipants);
        if(waitingListFromCallObject.length){  // we are receiving waiting participants in _waitingParticipants key under callObject.
          ...
          waitingListFromCallObject.forEach((item:any)=>{  // iterating each waiting participant
            ...
            try{
                // Giving permission to the waiting participant
            }catch(e){
                console.log(e) // We are getting thrown this error // Error: updateWaitingParticipant() only supported for joined meetings 
            }
