...
import api from "../../apiAgendaSimulive";
...
const VideoScreenAgendaQASimulive = (props) => {
  console.log("VideoScreen QA Simulive props", props);
  ...
  useEffect(() => {
    ...
    function handleNewMeetingState(event?: DailyEventObject) {
      ...
      if (event && event.action && event.action == "joined-meeting") {
        ...
        Axios.post(`${api_url.chatHistory}`, formData)
          .then((res) => {
            ...
            temp = res.data.data.map((item) => {
              ...
              //console.log("private", item.is_private_chat);
        ...
        console.log("participant qa formdata join ", formData2);

        Axios.post(`${api_url.daily_save_user_log}`, formData2)
          .then((res) => {
            console.log("participant qa Joined res", res);
          })
          .catch((err) => console.log("err", err));
        ...
        console.log("participant qa formdata left ", formData2);

        Axios.post(`${api_url.daily_save_user_log}`, formData2)
          .then((res) => {
            console.log("participant qa res left", res);
          })
          .catch((err) => console.log("err", err));
      }
      ...
      switch (callObject?.meetingState()) {
        case "joined-meeting":
          ...
          console.log("local qa simulive formdata join", formData);

          Axios.post(`${api_url.daily_save_user_log}`, formData)
            .then((res) => {
              console.log("local qa simulive join res", res);
            })
            .catch((err) => console.log("err", err));
        ...
        case "left-meeting":
          ...
          console.log("local qa simulive left", formData2);

          Axios.post(`${api_url.daily_save_user_log}`, formData2)
            .then((res) => {
              console.log("local qa simulive res left", res);
            })
            .catch((err) => console.log("err", err));
            ...
            >
              ...
              <Tray
                ...
                setShowQuestionPanel={(showQuestion) => {
                  console.log("join Q&A Question list", questionList);
                  setShowQuestionPanel(showQuestion);
                }}
...
export default connect(mapStateToProps)(VideoScreenAgendaQASimulive);
