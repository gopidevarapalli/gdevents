...
const VideoScreenNewAgenda = (props) => {
  console.log("VideoScreenNewAgenda props", props);
  console.log(
    "agenda_type",
    props.route.params.meeting_details.apidetails.agenda_type
  );
  ...
  useEffect(() => {
    if (
      props.route.params.meeting_details.apidetails.agenda_type == "roundtable"
    ) {
      setRoomConfig(
  ...
  useEffect(() => {
    ...
    // alert(props.route.params.provider)
    ...
    function handleNewMeetingState(event?: DailyEventObject) {
      ...
      if (event && event.action && event.action == "joined-meeting") {
        ...
        Axios.post(`${api_url.chatHistory}`, formData)
          ...
          // Axios.get(`${api_url.chatHistory}`,{
          //   params:{ cookie:props.login.cookie,
          //   event_id:props.event.common.event.event_id_single,
          //   meeting_id:props.route.params.meeting_details.id,
          //   meeting_type: (props.route.params.meeting_details.apidetails.select_speakers.indexOf(
          //     props.login.common.user.id.toString()
          //   ) > -1 || props.event.common.event.event_info.user_role == "presenter"  )?(props.route.params.meeting_details.apidetails.agenda_type == 'roundtable')?'agendaRoundtable':'agendaWebinarBackstage':(props.route.params.meeting_details.apidetails.agenda_type == 'roundtable')?'agendaRoundtable':'agendaWebinar'
        ...
        if (
          ...
          formData2.append("page_type", "agendaRoundtable");
        } else if (
          props.route.params.meeting_details.apidetails.select_speakers.indexOf(
            props.login.common.user.id.toString()
          ) > -1 ||
          ...
          formData2.append("page_type", "agendaWebinarBackstage");
        } else {
          formData2.append("page_type", "agendaWebinar");
        }
        formData2.append(
          "page_id",
          props.route.params.meeting_details.apidetails.agend_id
        ...
        console.log("participant formdata join ", formData2);

        Axios.post(`${api_url.daily_save_user_log}`, formData2)
          .then((res) => {
            console.log("participant join res", res);
          })
          .catch((err) => console.log("err", err));
        ...
        console.log("participant formdata left ", formData2);

        Axios.post(`${api_url.daily_save_user_log}`, formData2)
          .then((res) => {
            console.log("participant left res", res);
          })
          .catch((err) => console.log("err", err));
      }
      switch (callObject?.meetingState()) {
        case "joined-meeting":
          ...
          if (
            ...
            formData.append("page_type", "agendaRoundtable");
          } else if (
            props.route.params.meeting_details.apidetails.select_speakers.indexOf(
              props.login.common.user.id.toString()
            ) > -1 ||
            ...
            formData.append("page_type", "agendaWebinarBackstage");
          } else {
            formData.append("page_type", "agendaWebinar");
          }
          formData.append(
            "page_id",
            props.route.params.meeting_details.apidetails.agend_id
          ...
          console.log("local formdata join", formData);

          Axios.post(`${api_url.daily_save_user_log}`, formData)
            .then((res) => {
              console.log("local join res", res);
            })
            .catch((err) => console.log("err", err));
        ...
        case "left-meeting":
          ...
          console.log("local formdata left ", formData2);

          Axios.post(`${api_url.daily_save_user_log}`, formData2)
            .then((res) => {
              console.log("local left res", res);
            })
            .catch((err) => console.log("err", err));
  ...
  useEffect(() => {
    ...
    function handleAppMessage(event?: DailyEventObjectAppMessage) {
      ...
      if (event) {
        ...
        if (event.data.data.is_private !== true) {
          messagesList.push({
        ...
        // let questionandAnsList = questionList.length;
        // console.log("Q & A  list", questionandAnsList);
        // setAskQuestions(questionandAnsList);
      }
    }
   ...
   */
  const createRoom = (event, userdetails, meeting_details) => {
    console.log("create room called");
    setRoomCreateError(false);
    setAppState(AppState.Creating);
    api
      .createRoom(event, props.login, meeting_details)
      .then((room) => {
        ...
        <View style={styles.container}>
          {showCallPanel ? (
            ...
            >
              ...
              {/* <CallPanel roomUrl={roomUrl || ""} meetingDetails={props.route.params} event={props.event} /> */}
              <Tray
                ...
                setShowMsgPanel={(showMsgPanel) => {
                  console.log("message listt", messagesList);
                  setShowMsgPanel(showMsgPanel);
                }}
                showMsgPanel={showMsgPanel}
                setShowQuestionPanel={(showQuestion) => {
                  console.log("Question listttttt", questionList);
                  setShowQuestionPanel(showQuestion);
                }}
...
export default connect(mapStateToProps)(VideoScreenNewAgenda);
