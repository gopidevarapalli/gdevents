...
const VideoScreenNew = (props) => {
  ...
  useEffect(() => {
    ...
    createRoom(props.login.cookie,props.route.params.meeting_id, props.route.params.token); 
   ...
   */
  const createRoom = (cookie,meeting_id, token) => {
    setRoomCreateError(false);
    setAppState(AppState.Creating);
    api
      .createRoom(cookie,meeting_id, token)
      .then((room) => {
