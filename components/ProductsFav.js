...
const ProductsFav = (props) => {
  ...
  useEffect(() => {
    ...
    Axios.post(`${api_url.myFavourites}`, formData).then((res) => {
      //console.log("products favorites", res.data.products);
      setFavourites(res.data.products ? res.data.products : []);
      setIsLoading(false);
    });
  }, []);
...
export default connect(mapStateToProps)(ProductsFav);
